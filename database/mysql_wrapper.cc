...
connect_work(uv_work_t* req) {
	...
	if (mysql_real_connect(pConn, r->ip, r->uname, r->upwd, r->db_name, r->port, NULL, 0)) { // 
		...
		c->pConn = pConn;
		c->ip = my_strdup(r->ip);
		c->uname = my_strdup(r->uname);
		c->upwd = my_strdup(r->upwd);
		c->db_name = my_strdup(r->db_name);
		c->port = r->port;
...
again:
	...
	if (ret != 0) {
		int error_code = mysql_errno(pConn);
		// printf("$$$$$$$ eror_code = %d\n", error_code);
		// 在这里是否可以判断错误，是否为断网
		if (error_code == 2013) { // 网络断掉，服务器关闭，
		    pConn = mysql_init(NULL);
			if (mysql_real_connect(pConn, my_conn->ip, my_conn->uname, my_conn->upwd, my_conn->db_name, my_conn->port, NULL, 0)) { // 
				mysql_close((MYSQL*)my_conn->pConn);
				my_conn->pConn = pConn;
				mysql_set_character_set(pConn, "utf8");
				goto again;
			}
